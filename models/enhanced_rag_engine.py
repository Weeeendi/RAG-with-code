...
不要预设问题必须归类到某个固定范围。根据问题本质判断行动：

**类型1：协议/概念定义类**
- 问法："XX是什么"、"XX的含义"、"XX的信息"、"XX的数据结构"
- 示例：FE01是什么？PGN 0x00FE01是干什么的？骑行记录的数据格式？
- 行动：**直接使用 search_knowledge** 搜索相关协议/概念
- 注意：如果搜索结果为0，尝试扩展词（如"0x00" + FE01、PGN + 控制器匹配信息）

**类型2：操作/流程类**
- 问法："XX如何操作"、"如何上报"、"怎么配置"、"XX的流程"
- 示例：骑行记录上报如何操作？数据如何下发？
- 行动：**直接使用 search_knowledge**，不要先搜 DP
- 原因：操作类问题的答案在协议文档/业务流程中
...
**关键原则**：
- 通用知识（协议定义、数据结构）→ 用 search_knowledge
- DP数据点定义 → 用 search_dp_by_name / lookup_dp_id
- 代码实现 → 用 search_code
...
class ReActRAGEngine:
    ...
    def get_incomplete_info_reason(self, history: List[Dict]) -> str:
        ...
        import re
        reasons = []
        question = history[0].get('question', '') if history else ''
        ...
        for h in history:
            if h.get('results'):
                for r in h.get('results', []):
                    ...
                    content_tokens = set(re.findall(r'[a-zA-Z0-9]+', content))
                    overlap = question_tokens & content_tokens
                    if len(overlap) >= 2 or any(q in content or q in title for q in question_tokens if len(q) > 3):
                        relevant_results.append(r)

        if has_results and not relevant_results:
            reasons.append("检索结果与问题不相关，需要重新检索")
        elif has_results and relevant_results and len(relevant_results) < 3:
            reasons.append(f"相关结果不足({len(relevant_results)}条)，考虑扩展检索")
...
## 回答要求
请根据上述检索结果回答用户问题。如果检索到了相关信息，请引用来源并详细说明。
如果未能检索到相关信息，明确告知用户"根据现有资料未找到相关信息"，并建议用户提供更多上下文。

## 上下文验证（重要）
检查检索结果中的代码来源（subsystem字段）：
- 'gil/gps' 或 'GPS定位' 相关内容 = GPS模块状态，不是整车电源状态
- 'can/vehicle' 相关内容 = 整车CAN通信状态
- 'cil/ble' 相关内容 = 蓝牙通信状态

如果用户问的是"整车开机状态"但检索到的是GPS/定位相关内容，说明检索结果不匹配，应明确告知用户"未找到相关信息"而非强行回答。

## CAN协议ID方向判断
根据数据来源判断CAN ID的方向：
- 数据来自**外部设备（如整车控制器、BMS）** → RX接收，用于数据采集/状态同步
- 数据来自**IOT/VBOX** → TX发送，用于下发控制指令
- 同一问题中不得混用不同方向的ID，引用某个CAN ID时必须确认其方向

## 回答格式（必须包含三要素）
【触发机制】
<描述业务触发条件，何时触发、如何触发，引用来源>

【数据结构】
<描述数据格式，DP定义、字段含义、协议定义，引用来源>

【技术推论】
<如果协议未明确，基于代码推断的结论，或"无">

如果知识库中未找到相关信息，回答：
"根据现有资料，未找到关于'{state['question']}'的详细信息。"
"""
